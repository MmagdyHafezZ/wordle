...
const wordDictionary = [
  ...
  {
    word: "belt",
    hint: "A strip of leather or other material worn around the waist",
  },
...
function init() {
  ...
  keyboardpresses();
  enableTouch(); // enable touch functionality
}
...
function keyboardpresses() {
  currentbox = document.getElementById(
    "charBox." + gameState.currentRow + "" + gameState.currentCol
  );
...
function deleteletter() {
  if (gameState.currentCol === 0) {
    ...
    let prev = document.getElementById(
      "charBox." + gameState.currentRow + "" + (gameState.currentCol - 1)
    );
...
function clearGameGrid() {
  for (let i = 0; i < 4; i++) {
    for (let j = 0; j < 4; j++) {
      ...
      charBox.classList.remove("correct");
      charBox.classList.remove("contains");
...
function getTouchedBox(touch) {
  const x = touch.clientX;
  const y = touch.clientY;

  const elements = document.elementsFromPoint(x, y);
  for (let element of elements) {
    if (element.classList.contains("charBox")) {
      return element;
    }
  }
  return null;
...
function enableTouch() {
  ...
  document.body.addEventListener("touchstart", (e) => {
    ...
    selectedBox = getTouchedBox(touch);
    if (selectedBox) {
  ...
  document.body.addEventListener("touchmove", (e) => {
    e.preventDefault(); // Prevent scrolling while moving finger on grid
  });

  document.body.addEventListener("touchend", () => {
    if (selectedBox) {
      // Show the soft keyboard to let user choose a letter
      keyboard.style.display = "block";
      selectedBox = null; // Reset the selected box
    }
  });
}
function createSoftKeyboard() {
  const keyboard = document.createElement("div");
  keyboard.id = "soft-keyboard";
  ...
  keyboard.style.display = "none"; // Hide it initially
}
